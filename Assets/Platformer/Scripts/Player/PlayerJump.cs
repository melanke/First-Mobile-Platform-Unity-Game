...
namespace Acrocatic {
	...
	public class PlayerJump : MonoBehaviour {
		...
		void Update () {
			...
			if(!jump && CnInputManager.GetButtonDown("Jump") && jumps > 0 && !player.dashing && !player.sliding && !player.onLadder && (!player.crouching || (player.crouching && player.AllowedToStandUp()))) {
		...
		void FixedUpdate () {
			...
			if(jump) {
				...
				if (jumpType == JumpType.HoldToJumpHigher) {
					...
					} else if (CnInputManager.GetButtonDown("Jump") && jumpTimer > 0) {
